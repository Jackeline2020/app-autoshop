...
func TestVehicleIntegration_CreateAndRetrieve(t *testing.T) {
	...
	customer, err := customerUseCase.Create("Dono do Carro", "052.573.070-22", "", "dono@email.com", "11999999999", address)
	require.NoError(t, err)

	vehicle, err := vehicleUseCase.Create(customer.ID, "FFF6666", "Fiat", "Uno", 2020)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	assert.Equal(t, "FFF6666", found.Plate)
}
...
func TestVehicleIntegration_GetByCustomerID(t *testing.T) {
	...
	customer, err := customerUseCase.Create("Dono de Vários Carros", "216.243.760-02", "", "varios@email.com", "11999999999", address)
	require.NoError(t, err)
...
func TestVehicleIntegration_Update(t *testing.T) {
	...
	customer, err := customerUseCase.Create("Dono Atualiza", "364.649.480-83", "", "atualiza@email.com", "11999999999", address)
	require.NoError(t, err)
...
func TestVehicleIntegration_Delete(t *testing.T) {
	...
	customer, err := customerUseCase.Create("Dono Deleta", "263.470.310-43", "", "deleta@email.com", "11999999999", address)
	require.NoError(t, err)
